...
ggplot(top_countries, aes(x = fct_reorder(Country, Arrivals), y = (Arrivals)*1000, fill = Arrivals)) +
  geom_col() +
  coord_flip() +
...
ggplot(long_data_four, aes(x = Year, y = Arrivals, group = Country, color = Country)) +
  ...
  labs(title="Visits since 1995 in most popular countries")
```
...
```{r}
happiness_data <- happy_ten[,7:11]
happiness_data <- na.omit(happiness_data)
pca_results <- prcomp(happiness_data, scale. = TRUE)
